...
From the repository root, `docker compose up -d --build` starts the web, API, relay, and worker together using the root `.env`. For a standalone worker checkout, copy `.env.example` to `.env`, set `RVS_API_BASE_URL` and `RVS_WORKER_TOKEN`, then run `docker compose up -d --build` from `apps/worker`. The worker compose also reads the repository root `.env` first, then `apps/worker/.env` if present. Omit `RVS_WORKER_ID` to generate `worker-<hostname>` automatically. The default same-host API URL is `http://host.docker.internal:3200`.
