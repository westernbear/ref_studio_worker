...
describe("worker config", () => {
  it("parses required values and safe interval defaults", () => {
    const config = parseWorkerConfig({ RVS_API_BASE_URL: "https://api.example.test/", RVS_WORKER_TOKEN: "secret", RVS_WORKER_CAPABILITIES: "compiler,renderer" })
  ...
  it("rejects missing token and unsafe intervals", () => {
    expect(() => parseWorkerConfig({ RVS_API_BASE_URL: "https://api.example.test", RVS_HEARTBEAT_INTERVAL_MS: "99" })).toThrow()
  })
})
